...
public class MinusOp implements Polynomial{
	...
	public MinusOp(Polynomial v, Polynomial n) {
...
//			System.out.println("s");
		...
		if(one.hasNoVariables() && two.hasNoVariables() && !onei.hasNext() && !twoi.hasNext()) {
			...
			Polynomial oneone = new Number(1); 
			...
			if(one.equals(oneone)) {
				
				n = two; 
			}
			if(two.equals(oneone)) {
...
//		Polynomial f = new MinusOp(one, two); 
//		ss.add(f);
		ss.add(one);
		ss.add(two);
