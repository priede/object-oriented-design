...
public class PlusOp implements Polynomial{
	...
	public PlusOp(Polynomial v, Polynomial n) {
	...
	public int evaluate(Map<String, Integer> m) {
		...
		while(onei.hasNext()) {
			result = result + (onei.next().evaluate(m)); 
		}
		while(twoi.hasNext()) {
			result = result + (twoi.next().evaluate(m)); 
		}

		if(m.containsKey(one.toString())) {

			result = result + m.get(one.toString());
			result = result + Integer.parseInt(two.toString());
		}
		else if (m.containsKey(one.toString())) {

			result = result + m.get(two.toString());
	...
	public boolean equals(Object obj) {
		...
		if(obj instanceof Polynomial ) {
			...
			ArrayList<Character> objj = new ArrayList<Character>();
			for (Object o : obj.toString().toCharArray()) {
				objj.add((Character) o);
			}
			
//			System.out.println("s1 " + chars.size());
//			System.out.println("s1 " + objj.size());
			if(chars.size() != objj.size()) {
//				System.out.println("sadf");
	...
	public Iterator<Polynomial> childIterator() {
		...
		Iterator<Polynomial> i = one.childIterator();
		while(i.hasNext()) {
			ss.add(i.next()); 
		}

//		Polynomial f = new PlusOp(one, two); 
		ss.add(one);
		ss.add(two);
